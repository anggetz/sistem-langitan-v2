...
use App\Models\KegiatanAkdEksKelompok;
...
class KegiatanKelompokController extends Controller
{
    public function Create(Request $request)
    {
        try {
            $request->validate([
                'nm_kelompok_kegiatan' => 'required|string|max:255',
                'is_kemahasiswaan' =>  'required',
                'is_akademik' =>  'required',
            ]);
            ...
            return response()->json([
                'message' => Message::OK,
                'data' => 'Kegiatan Kelompok Berhasil Dibuat',
            ], 201);
        } catch (\Exception $e) {
            return response()->json([
                'message' => 'Gagal  membuat Kegiatan Kelompok',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
